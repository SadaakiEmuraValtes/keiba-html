...
// ---- 選択状態 ----
// 通常: normalSel[pos] = 馬番 or null
const normalSel = ref([null, null, null])
// ボックス: boxSel = 馬番の配列
const boxSel = ref([])
// フォーメーション: formSel[pos] = 馬番の配列
const formSel = ref([[], [], []])
// 流し軸(1頭): nagashiAxis = 馬番 or null
const nagashiAxis = ref(null)
// 流し軸(2頭): nagashiAxis2 = 馬番の配列
const nagashiAxis2 = ref([])
// 流し相手: nagashiLegs = 馬番の配列
...
// ---- 流しUI定義 ----
// returns array of { label, isAxis, isAxis2, isLeg } per column
...
const combinations = computed(() => {
  ...
  if (m === '通常') {
    ...
    if (new Set(sel).size < n) return []  // 重複不可
    return [sel]
  }
...
function horseName(num) {
  return race.value?.horses.find(h => h.number === num)?.name ?? ''
}
...
function numpadPress(d) {
  ...
  const next = parseInt((s === '0' ? '' : s) + d, 10) * 100
  if (!isNaN(next) && next <= store.balance) betAmount.value = next
}
...
const isAllSelected = computed(() => race.value?.horses.every(h => boxSel.value.includes(h.number)))
function toggleAllBox() {
  if (isAllSelected.value) boxSel.value = []
  else boxSel.value = race.value?.horses.map(h => h.number) ?? []
}
</script>
...
<template>
  <div v-if="race">
    ...
    <div class="race-header card mb-12">
      <div class="race-header-row">
        <span class="race-venue-label">{{ race.venueName }} {{ race.round }}R</span>
        <span class="badge" :class="'badge-' + race.status">{{ statusLabel[race.status] }}</span>
      </div>
      <div class="race-name-big">{{ race.name }}</div>
    ...
    <div v-if="race.result" class="card mb-12">
      ...
      <div class="result-box">
        <div v-for="(key, i) in ['first','second','third']" :key="key" class="result-place">
          ...
          <div class="place-num" :class="['first-place','second-place','third-place'][i]">{{ race.result[key] }}番</div>
          <div class="place-name">{{ horseName(race.result[key]) }}</div>
        </div>
      </div>
    </div>
    ...
    <template v-if="race.status === 'open'">
        ...
        <div class="table-scroll">
          <table class="horse-table">
            ...
            <tbody>
              ...
              >
                <td><span class="frame-badge" :class="'frame-' + ((horse.frame - 1) % 8 + 1)">{{ horse.frame }}</span></td>
                ...
                <template v-else-if="method === '流し'">
                  <td v-for="(col, ci) in nagashiCols" :key="ci">
                    <template v-if="col.isAxis">
                      <button
                        class="sel-btn sel-axis" :class="{ active: nagashiAxis === horse.number }"
                        @click="toggleAxis(horse.number)"
                      >{{ horse.number }}</button>
                    </template>
                    <template v-else-if="col.isAxis2">
                      <button
                        class="sel-btn sel-axis2" :class="{ active: nagashiAxis2.includes(horse.number) }"
                        @click="toggleAxis2(horse.number)"
                      >{{ horse.number }}</button>
                    </template>
                    <template v-else>
                      <button
                        class="sel-btn sel-leg" :class="{ active: nagashiLegs.includes(horse.number) }"
                        @click="toggleLeg(horse.number)"
                      >{{ horse.number }}</button>
                    </template>
                  </td>
                </template>
      ...
      <!-- 全通りチェックボックス (ボックス時) -->
      <div v-if="method === 'ボックス' && boxSel.length > 0" class="allcheck-row mb-8">
      ...
      <div class="purchase-panel card mb-12">
        <div class="purchase-top">
          ...
          <div class="numpad-area">
            <div class="numpad-label">全額（100円単位）</div>
            <div class="numpad-display">{{ (betAmount / 100).toLocaleString() }}<span class="numpad-unit">00円</span></div>
          ...
          <div class="purchase-right">
            ...
            <div class="purchase-row">
              <span class="purchase-row-label">{{ combinations.length }}通り</span>
            </div>
        ...
        <button
          ...
          @click="placeBet"
        >
          {{ canBet ? `${combinations.length}通り ¥${totalAmount.toLocaleString()} 購入する` : '馬を選択してください' }}
        </button>
    ...
    <!-- 締切・発走前・確定時メッセージ -->
    <div v-else-if="race.status === 'upcoming'" class="status-notice card">
      発走前のレースです（{{ race.startTime }} 発走予定）
    </div>
    <div v-else-if="race.status === 'closed'" class="status-notice card">
      投票を締め切りました。結果をお待ちください。
    </div>

    <!-- 出走馬テーブル（結果確定・締切等でも表示） -->
    <div class="card">
      ...
      <div class="table-scroll">
        <table class="horse-table">
          ...
          <tbody>
            <tr v-for="h in race.horses" :key="h.number">
              <td><span class="frame-badge" :class="'frame-' + ((h.frame - 1) % 8 + 1)">{{ h.frame }}</span></td>
  ...
  <div v-else class="card">レースが見つかりません。
...
.race-meta-row { display: flex; gap: 12px; font-size: 0.8rem; color: #888; flex-wrap: wrap; align-items: center; }
...
.sel-axis2.active { background: #7c3aed; }
...
.numpad-unit { font-size: 0.72rem; color: #888; margin-left: 3px; }
...
.purchase-balance { font-size: 0.78rem; color: #888; margin-bottom: 8px; }
...
.status-notice { color: #888; font-size: 0.9rem; margin-bottom: 16px; }

.allcheck-row { display: flex; align-items: center; gap: 8px; }
