...
| 残高チャージ | 仮想銀行振込でデモ用残高を追加（実際の入金なし）。マイページ and 投票画面からモーダルで操作可能 |
...
- 各日程の重賞レース5本（10〜11R）が対象（`getWin5RaceKeys(dateIdx)` で取得）。最終レースは最高グレード（GI）
- トップページのスケジュール表下に5レースを横並びで組み込み。ナビメニューからは削除
