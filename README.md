...
| スケジュール表 | 競馬場×12Rのグリッド。全セル均一サイズ・クリック可能（出馬表・投票・結果）。重賞セルにはGI/GII/GIIIチップ表示 |
| 重賞一覧 | スケジュール表の下部に重賞レース（GI/GII/GIII）を締め切り時刻順で表示。各競馬場最大2レース（10〜11R） |
| イベント通知 | 出走取消・除外・騎乗変更・競走中止が時刻連動で発生。TOPのスケジュール表下に一覧表示。競走中止セルはグレーアウト |
| 自動払い戻し（イベント） | 出走取消・除外の対象馬を含む馬券コンボを自動返金。競走中止は対象レースの全馬券を全額払い戻し |
| 出馬表タブ | 出走馬の詳細（枠・番・馬名・騎手・性齢・体重・人気・オッズ）。取消・除外馬は打ち消し線、騎乗変更は新旧騎手名を表示 |
...
| **WIN5** | トップページのスケジュール表下に組み込み。日程の重賞5レース（10〜11R）の1着馬を当てる特別式。5レースを横並びで表示し馬を縦に選択。複数頭選択可（ボックス購入）。全レース確定後に自動精算 |
| 残高チャージ | 仮想銀行振込でデモ用残高を追加（実際の入金なし）。マイページおよび投票画面からモーダルで操作可能 |
| 自動精算 | 仮想時刻を進めるとレース確定・購入馬券が自動精算 |
| マイページ | 残高・収支サマリー・投票履歴（的中/ハズレ/結果待ち/払戻） |
...
├── store/
│   └── index.js              # グローバルストア・自動精算・イベント払い戻しロジック
├── data/
│   ├── scenarios.js          # JRA競馬場定義・5日程シナリオ・raceKeyエンコード・WIN5/イベント定義
│   ├── masterData.js         # 馬/結果の決定論的生成・全着順・払戻計算
│   └── users.js              # テストアカウント5件
└── views/
    ├── HomeView.vue          # スケジュール表・重賞一覧・WIN5セクション・イベント一覧
...
各日程に4イベントが設定されています（`scenarios.js` の `events` 配列）。

| イベント種別 | 財務処理 | 表示 |
|------------|---------|------|
| 出走取消（scratch） | 該当馬を含むコンボを返金 | 出馬表で打ち消し線 |
| 除外（exclusion） | 該当馬を含むコンボを返金 | 出馬表で打ち消し線 |
| 騎乗変更（jockey_change） | なし | 出馬表で新旧騎手名を表示 |
| 競走中止（race_cancel） | 全馬券を全額払い戻し | スケジュール表セルをグレーアウト |

- イベントは `triggerHour`（9〜12時）で発動。仮想時刻を進めると順次出現
- 各アカウントで `processedEventIds` を管理し、払い戻しの重複処理を防止

---

## WIN5 仕様

- 各日程の重賞レース5本（10〜11R）が対象（`getWin5RaceKeys(dateIdx)` で取得）。最終レースは最高グレード
- トップページのスケジュール表下に5レースを横並びで組み込み
- 各レースで1頭以上を選択、組み合わせ数 = 選択頭数の積
